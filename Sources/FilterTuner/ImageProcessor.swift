...
class ImageProcessor {
    ...
    // Adaptive Thresholdingのカーネル（絶対輝度フロア付き）
    // original: DivisionNorm後の画像（影除去済み）
    // darkFloor: DivNorm後でこの輝度以下なら問答無用でテキスト判定（塗りつぶし保護）
    // 影はDivNormで白(≈0.90)に正規化済みなので誤判定しない
    private lazy var adaptiveThresholdKernel: CIColorKernel? = {
        let kernelSource = """
        kernel vec4 adaptiveThreshold(__sample pixel, __sample blurred, __sample original, float offset, float strength, float darkFloor) {
            ...
            // DivisionNorm後の輝度と彩度をチェック
            float origBrightness = dot(original.rgb, vec3(0.299, 0.587, 0.114));
            ...
            // 通常のAdaptive判定
            // OR 暗い AND 低彩度（=黒インクの塗りつぶし）→ テキスト
            // 高彩度の色文字（緑ペン等）はdarkFloorから除外し、色復元に任せる
    ...
    // 色味復元カーネル: ハイブリッド方式
    // - divNormRef（DivisionNorm後）: 彩度判定に使用 → 影の色かぶりを正確に除外
    // - colorRef（元画像）: 色抽出に使用 → 鮮やかな色情報を保持
    private lazy var colorPreservationKernel: CIColorKernel? = {
        let kernelSource = """
        kernel vec4 colorPreserve(__sample processed, __sample colorRef, __sample divNormRef, float satThreshold, float preservation, float boost) {
            // divNormRef（影除去済み）の彩度で判定
            // DivisionNormは影の暖色かぶりを除去するため、
            // 影エリアの彩度≈0（通過しない）、スタンプ等の本当の色は彩度が高いまま（通過する）
            ...
            if (saturation > satThreshold) {
                ...
                // colorRef（元画像）から色比率を抽出（鮮やかな色情報）
                float refLuma = dot(colorRef.rgb, vec3(0.299, 0.587, 0.114));

                // 輝度で正規化して色比率（クロミナンス）を抽出
                vec3 chromaRatio = refLuma > 0.01 ? colorRef.rgb / refLuma : vec3(1.0);

                // 彩度ブースト: グレー(1,1,1)からの偏差を増幅
                vec3 boostedChroma = mix(vec3(1.0), chromaRatio, boost);

                // processed画像の輝度を取得
                float processedLuma = dot(processed.rgb, vec3(0.299, 0.587, 0.114));

                // 二値化で黒くされた色付きエリアの輝度を復元
                float lumaFloor = refLuma * 0.5;
                float effectiveLuma = max(processedLuma, lumaFloor);

                // クロミナンスを輝度に適用して色を復元
                vec3 colorized = effectiveLuma * boostedChroma;
                colorized = clamp(colorized, 0.0, 1.0);

                vec3 result = mix(processed.rgb, colorized, blendFactor);
                return vec4(result, 1.0);
            }
    ...
    private func applyDocumentScanFilter(to ciImage: CIImage, params: FilterParams, debug: Bool, outputDir: String) -> CIImage? {
        ...
        let colorReference = currentImage
        ...
        // ★ 色味復元用: DivisionNorm後を保存（影の色かぶりが除去済み → 正確な彩度判定に使用）
        let divNormReference = currentImage
        ...
        // Step 5: Adaptive Thresholding（DivisionNorm後の輝度フロア付き）
        // divNormReferenceを使用: 影は白に正規化済み(≈0.90)→誤判定しない
        // 塗りつぶし部分はDivNorm後もまだ暗い(≈0.5-0.7)→正しく検出
        currentImage = applyAdaptiveThreshold(to: currentImage, original: divNormReference, params: params) ?? currentImage
        ...
        // Step 7: 色味復元（colorPreservation > 0 の場合のみ適用）
        if params.colorPreservation > 0 {
            currentImage = applyColorPreservation(to: currentImage, colorReference: colorReference, divNormReference: divNormReference, params: params) ?? currentImage
            saveDebug(currentImage, step: "07_ColorPreserve")
        }

        // Step 8: 最終コントラスト強調（黒を締め、白を飛ばす）
        if params.finalContrast > 1.0 {
            currentImage = applyFinalContrast(to: currentImage, params: params) ?? currentImage
            saveDebug(currentImage, step: "08_FinalContrast")
        }
    ...
    /// Adaptive Thresholding（元画像の絶対輝度フロアによる塗りつぶし保護付き）
    private func applyAdaptiveThreshold(to ciImage: CIImage, original: CIImage, params: FilterParams) -> CIImage? {
    ...
    /// 色味復元: divNormRefで彩度判定、colorRefから色抽出のハイブリッド方式
    private func applyColorPreservation(to ciImage: CIImage, colorReference: CIImage, divNormReference: CIImage, params: FilterParams) -> CIImage? {
        ...
        guard let result = kernel.apply(
            ...
            arguments: [
                ...
                colorReference,
                divNormReference,
                params.colorSaturationThreshold,
    ...
    /// 最終コントラスト強調: 黒を締め、白を飛ばしてAdobe風のくっきり感を出す
    private func applyFinalContrast(to ciImage: CIImage, params: FilterParams) -> CIImage? {
        let colorControls = CIFilter.colorControls()
        colorControls.inputImage = ciImage
        colorControls.contrast = params.finalContrast
